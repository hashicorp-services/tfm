...
When an agent creates or submits any of the following, it **must** identify itself as the author in the body:

- Pull requests
- Issues
- Issue comments
- Commit messages (where a body is included)

**Required footer for PR and issue bodies:**

```
---
> 🤖 Submitted by **<agent-name>** (AI agent) on behalf of @<github-username>
```

**Required footer for issue comments:**

```
---
> 🤖 Posted by **<agent-name>** (AI agent) on behalf of @<github-username>
```

Rules:

- Always append the attribution footer as the last element of the body.
- Replace `<agent-name>` with the name of the active AI agent (e.g. the model, assistant, or tool name as it identifies itself).
- Replace `<github-username>` with the authenticated GitHub user at submission time: `gh api user --jq .login`.
- Do not omit the footer even when the content is brief.
- Do not add the footer to automated CI/CD comments generated by GitHub Actions workflows — only to content directly authored by an agent in this workspace.
